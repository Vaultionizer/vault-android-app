...
class CreatePCViewModel @Inject constructor(
    ...
    private val _pcCreationRes = MutableLiveData<InputPCNameFormState>()
    val pcCreationRes: LiveData<InputPCNameFormState> = _pcCreationRes
    ...
    fun pcNameChanged(text: String) {
        ...
        if (name.length < 4) {
            _pcCreationRes.value = InputPCNameFormState(R.string.create_pc_too_short_name, false)
        } else {
            _pcCreationRes.value = InputPCNameFormState(null, true)
        }
    }
